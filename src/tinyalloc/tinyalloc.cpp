...
bool tinyalloc::ta_init(HeapAllocator *a, const void *base, const void *limit, const size_t heap_blocks, const size_t split_thresh, const size_t alignment)
{
    a->heap = (Heap *)base;
    a->heap_limit = limit;
...
size_t tinyalloc::ta_blocksize(HeapAllocator *a, void *ptr)
{
...
bool tinyalloc::ta_free(HeapAllocator *a, void *free)
{
    Block *block = a->heap->used;
    Block *prev = NULL;
    while (block != NULL)
...
void *tinyalloc::ta_alloc(HeapAllocator *a, size_t num)
{
...
void *tinyalloc::ta_calloc(HeapAllocator *a, size_t num, size_t size)
{
    num *= size;
    Block *block = alloc_block(a, num);
    if (block != NULL)
...
size_t tinyalloc::ta_num_free(HeapAllocator *a)
{
    return count_blocks(a->heap->free);
}

size_t tinyalloc::ta_num_used(HeapAllocator *a)
{
    return count_blocks(a->heap->used);
}

size_t tinyalloc::ta_num_fresh(HeapAllocator *a)
{
    return count_blocks(a->heap->fresh);
}

bool tinyalloc::ta_check(HeapAllocator *a)
{
